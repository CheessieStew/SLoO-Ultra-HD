...
float mdaMultiBand::getParameter(VstInt32 index)
{
	float v=0;
...
void mdaMultiBand::getParameterName(VstInt32 index, char *label)
{
	switch(index)
  {
...
void mdaMultiBand::getParameterLabel(VstInt32 index, char *label)
{
	switch(index)
  {
...
void mdaMultiBand::process(float **inputs, float **outputs, VstInt32 sampleFrames)
{
	float *in1 = inputs[0];
	float *in2 = inputs[1];
...
void mdaMultiBand::processReplacing(float **inputs, float **outputs, VstInt32 sampleFrames)
{
	float *in1 = inputs[0];
	float *in2 = inputs[1];
	float *out1 = outputs[0];
