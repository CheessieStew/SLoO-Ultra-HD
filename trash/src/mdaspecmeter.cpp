...
//
// Copyright(c)2002 Paul Kellett (mda-vst.com)
// Based on VST 2.0 SDK (c)1999 Steinberg Soft und Hardware GmbH, All Rights Reserved
//
...
float mdaSpecMeter::getParameter(VstInt32 index)  { return param[index]; }

void mdaSpecMeter::setProgram(VstInt32 program)
{
...
void mdaSpecMeter::setParameter(VstInt32 index, float value)
{
...
void mdaSpecMeter::getParameterName(VstInt32 index, char *label)
{
	switch (index)
	{
...
void mdaSpecMeter::getParameterDisplay(VstInt32 index, char *text)
{
  char string[16];
...
void mdaSpecMeter::getParameterLabel(VstInt32 index, char *label)
{
	switch (index)
	{
		case _PARAM0: strcpy(label, ""); break;
...
void mdaSpecMeter::process (float **inputs, float **outputs, VstInt32 sampleFrames)
{
  float *in1  = inputs[0];
  float *in2  = inputs[1];
...
void mdaSpecMeter::processReplacing (float **inputs, float **outputs, VstInt32 sampleFrames)
{
  float *in1  = inputs[0];
  float *in2  = inputs[1];
	float *out1 = outputs[0];
